...
class Animator:
    frameTime = 1/15
    def __init__(self,folderPath,size,name,loop = True):
        self.name = name
        self.frames = []
        self.size = size
        fileNames = []
                ...
                for char in name:
                    for i in range(10):
                        if char == str(i):
                            num += char
                smallNum = ""
                for char in smallestFileName:
                    for i in range(10):
                        if char == str(i):
                            smallNum += char
                if int(num) < int(smallNum):
                    smallestFileName = name
            sortedFileNames.append(smallestFileName)
            fileNames.remove(smallestFileName)
        print(sortedFileNames)
        for i in range(len(sortedFileNames)):
            path = folderPath + "/" + sortedFileNames[i]
        ...
        self.frameNumber = 0
        self.frameDelta = 0
        self.totalFrames = len(sortedFileNames)
...
if __name__ == "__main__":
    ...
    idle = Animator("Prototyping\Colin\playerIdleFrames",[48,64],"Idle")
